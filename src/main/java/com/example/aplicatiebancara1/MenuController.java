...
public class MenuController {
    ...
    @FXML
    private Text tranzactieText1;
    @FXML
    private Text tranzactieText2;
    @FXML
    private Text tranzactieText3;
    @FXML
    private Text tranzactieText4;
    ...
    private void updateTranzactii() {
        ...
        Text[] textElements = {tranzactieText1, tranzactieText2, tranzactieText3, tranzactieText4};
    ...
    private void onConfirmTrimite() {
        ...
        try {
            ...
            try (Connection conn = DatabaseConnection.getConnection()) {
                ...
                stmt = conn.prepareStatement("INSERT INTO TRANZACTII (ID_TRANZACTIE, ID_UTILIZATOR, ID_DESTINATAR, VALOARE_TRANZACTIE, DETALII_TRANZACTIE) VALUES (tranzactii_seq.NEXTVAL, ?, ?, ?, ?)");
        ...
        try {
            ...
            try (Connection conn = DatabaseConnection.getConnection()) {
                if (conn == null || conn.isClosed()) {
                    throw new SQLException("Conexiunea este null sau închisă");
                }
                System.out.println("Conexiune stabilită cu succes: " + !conn.isClosed());

                PreparedStatement stmt = conn.prepareStatement("SELECT PIN FROM UTILIZATORI WHERE ID_UTILIZATOR = ?");
                stmt.setString(1, utilizator.getIdUtilizator());
                System.out.println("Execut interogare SELECT PIN");
                ResultSet rs = stmt.executeQuery();
                if (rs.next()) {
                ...
                stmt.setString(2, utilizator.getIdUtilizator());
                System.out.println("Execut UPDATE UTILIZATORI");
                stmt.executeUpdate();
                ...
                System.out.println("Execut INSERT TRANZACTII");
                stmt.executeUpdate();
    ...
    private void onConfirmDepunere() {
        ...
        try {
            ...
            try (Connection conn = DatabaseConnection.getConnection()) {
                ...
                PreparedStatement stmt = conn.prepareStatement("SELECT PIN FROM UTILIZATORI WHERE ID_UTILIZATOR = ?");
                System.out.println("Setez ID_UTILIZATOR: " + utilizator.getIdUtilizator());
                stmt.setString(1, utilizator.getIdUtilizator());
                System.out.println("Execut interogare SELECT PIN");
                ResultSet rs = stmt.executeQuery();
                if (rs.next()) {
                ...
                stmt.setDouble(1, newBalance);
                stmt.setString(2, utilizator.getIdUtilizator());
                System.out.println("Execut UPDATE UTILIZATORI cu sold nou: " + newBalance);
                stmt.executeUpdate();
